...
3. **Contacts sync** — `listContacts` → cached in Aspera Connect → search + Call
...
3. Allow phone calls + contacts (+ battery unrestricted on OnePlus).
...
- `{"type":"helloAck","ok":true,"protocol":1,"capabilities":{"placeCall":true,"contacts":true,"mirror":false,"input":false}}`
